   ...
   public partial class FormOne : Form
   {
      ...
      void GetNonExistentItem(int id)
      {
         try
         {
            ...
            RichTextBoxReader.AppendText("Статус: ОШИБКА - элемент найден (не должно было произойти)");
            RichTextBoxReader.AppendText(Environment.NewLine);
         }
         catch (WebException ex)
         {
      ...
      void TestInvalidMethod()
      {
         try
         {
            ...
            TextBoxReader.AppendText(Environment.NewLine);
            ListBoxReader.Items.Add("Статус: ОШИБКА - сервер принял неразрешенный метод");
      ...
      void HandleWebException(WebException ex)
      {
         ...
         else
         {
            ...
            TextBoxReader.AppendText("Ошибка: {0}", ex.Message);
            TextBoxReader.AppendText(Environment.NewLine);
            ListBoxReader.Items.Add("Тело ошибки: " + errorBody);
            RichTextBoxReader.AppendText("Тело ошибки: " + errorBody);
            RichTextBoxReader.AppendText(Environment.NewLine);
